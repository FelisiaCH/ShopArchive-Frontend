...
Range picker, per-currency totals, takings by day, busiest hours, spending by category, and export.
Charts render one currency at a time behind a switcher.
...
widths, `npm run check` green. Assert at minimum that per-currency totals never combine currencies and
that charts show one currency at a time behind the switcher. Add a phase-specific list and state it in the
report._
