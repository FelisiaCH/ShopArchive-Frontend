...
Carved from the original F9 (Finishing), part 2 of 3. F9a and F9b are independent of each other; **F9c
...
Print stylesheets for the day summary and for a single entry, honouring the receipt language; **the
thermal print action, shown only when the server reports a configured printer** — a print button on an
entry and on the day summary, reporting its own outcome with a retry, and never implying the entry itself
failed.

## Verify

_No phase-specific verify list was given in the original plan. The universal Rules above still apply.
Assert at minimum that the print action appears only when the server reports a configured printer, that a
print failure reports itself with a retry and never implies the entry failed, and that print output
honours the receipt language. State what you verified in the report._
